...
let isBefore = (a: t, b: t): bool => {
  a->getTime < b->getTime;
};

let durationInHours = (~ends, ~starts) =>
  Js.Date.valueOf(ends) -. Js.Date.valueOf(starts) |> msToHours;
let slotDuration = 30.;
let slotSizeInMin = (~ends, ~starts, ~slotDuration) => {
  let s = Js.Date.valueOf(ends) -. Js.Date.valueOf(starts);
  let slotSize = s /. slotDuration |> msToMin |> int_of_float;
  slotSize;
};
